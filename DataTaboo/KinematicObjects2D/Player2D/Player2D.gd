...
func _ready():
	match mode:
		control_mode.ACCELERATION:
			control_function = "control_function_acceleration"
			
		control_mode.INSTANT:
			control_function = "control_function_instant"
...
func control_function_instant():
	velocity *= 0
	velocity = acceleration * Input.get_vector("ui_left", "ui_right", "ui_up", "ui_down")
